...
class DefaultLoginRepo @Inject constructor(
    ...
    private suspend fun checkSchoolLogin() = withContext(Dispatchers.IO) {
        ...
        if(firstLogin) true
            ...
            // 可能是Cookie过期
            // 判断是否存有账号和密码
            val sid = loginStatusManager.sid.get()
            val password = loginStatusManager.password.get()
    ...
    private suspend fun loginBIT101(username: String, password: String) = withContext(Dispatchers.IO) {
        ...
        loginStatusManager.fakeCookie.set(fakeCookie)
    ...
    override suspend fun login(username: String, password: String) = withContext(Dispatchers.IO) {
        ...
        if(res) {
            loginStatusManager.sid.set(username)
            loginStatusManager.password.set(password)
            loginStatusManager.status.set(true)
        }
        res
    }
